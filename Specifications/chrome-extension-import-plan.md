...
Chromeで開いているarXivページ、論文ページ、PDFタブから、DOI/arXiv ID/PDFをLegraへ渡し、Legra側で登録、リネーム、フォルダ分けを行う。
...
2. `chrome-extension/native-messaging/app.legra.importer.json` を編集する。
...
5. Legraを起動し、managed directoryを設定する。
...
5. `Import to Legra` を押す。
6. Legraを開く、またはメイン画面の `Import inbox` を押す。
...
```json
{
  ...
  "request": {
    ...
    "year": 2026,
    "publication": "Journal",
    "pdf_path": "/path/to/Downloads/paper-manager-import/file.pdf",
...
- [ ] arXiv import後、Legraの `Import inbox` で登録、リネーム、カテゴリ分けが完了する。
...
- [ ] Native MessagingやLegra側のImport失敗時、`Downloads/paper-manager-import/` の一時PDFが削除される。
- [ ] Native Messaging manifest未設定時、拡張popupに明確なエラーが出る。
- [ ] managed directory未設定時、Legra側で失敗し、requestが `failed/` に移動する。
